...
let productCache=[],productImages=[null,null,null],couponCache=[];
function renderProductImage(){const images=productImages.filter(Boolean);productImagePreview.innerHTML=images.length?images.map((value,i)=>`<img src="${value}" alt="Foto ${i+1}">`).join(''):'Sem imagens'}
...
document.querySelectorAll('.product-image-file').forEach(input=>input.onchange=()=>{const file=input.files[0],slot=Number(input.dataset.imageSlot);if(!file)return;if(file.size>10000000){productError.textContent='Cada imagem deve ter no máximo 10 MB.';input.value='';return}const reader=new FileReader();reader.onload=()=>{productImages[slot]=reader.result;renderProductImage()};reader.readAsDataURL(file)});
